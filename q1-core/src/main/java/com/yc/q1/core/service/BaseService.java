package com.yc.q1.core.service;
...
public interface BaseService<E> {
	...
	/**
	 * 通用列表导出excel reportExcel:(这里用一句话描述这个方法的作用). 可选).
	 *
	 * @param @param
	 *            tableCode
	 * @param @param
	 ...
	 * 执行HQL语句，获取实体类
	 * 
	 * 若hql语句中涉及到只查询1个字段，则直接返回该字段的对应类型的数据
	 * 
	 * 若hql语句中涉及到只查询2-N个字段，实际上是不能将结果集封装到实体类中，返回的结果是Object[]类型的数据
	 * 若想使结果集封装到实体类中，要在实体类中加入构造方法，再使用这种方式的hql语句：
	 * 
	 * select new XXX(field1,field2...) from XXX；
	 * 
	 * @param hql
	 *            SQL语句
	 * @param args
	 ...
	 * 根据sql查询转换成指定的实体合集
	 * 
	 * 若传入了clz参数，那么将会把sql语句中的字段一一对应的实体类；若sql中存在实体类中没有定义的字段，那么会报错
	 * 
	 * @param sql
	 *            SQL语句
	 * @param clz
