...
import com.yc.q1.core.model.extjs.ExtSortModel;
...
public class BaseDaoImpl<E> implements BaseDao<E> {
	...
	private <ICriterion> Criterion GetComparison(String comparison, String field, Object value) {
		...
		case "in": // 包含
			...
			break;
		case "not in": // 包含
			Object[] setInValue = value.toString().split(",");
			res = (ICriterion) Restrictions.not(Restrictions.in(field, setInValue));
			break;	
		default: // 默认为等于
			res = (ICriterion) Restrictions.eq(field, value);
	...
	public <T> List<T> queryEntityByHql(String hql) {
		return this.queryEntityByHql(hql, null);
	}
	...
	public List<E> queryByHql(String hql, String propName, Object[] objs) {
		return queryByHql(hql, 0, 0, propName, objs);
	}
