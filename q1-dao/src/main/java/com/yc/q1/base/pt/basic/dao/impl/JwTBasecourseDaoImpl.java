package com.yc.q1.base.pt.basic.dao.impl;

import org.springframework.stereotype.Repository;

import com.yc.q1.base.pt.basic.dao.JwTBasecourseDao;
import com.yc.q1.base.pt.basic.model.BaseCourse;
import com.zd.core.dao.BaseDaoImpl;

/**
 * 
 * ClassName: JwTBasecourseDaoImpl Function: TODO ADD FUNCTION. Reason: TODO ADD
 * REASON(可选). Description: 基础课程信息实体Dao接口实现类. date: 2016-03-13
 *
 * @author luoyibo 创建文件
 * @version 0.1
 * @since JDK 1.8
 */
@Repository
public class JwTBasecourseDaoImpl extends BaseDaoImpl<BaseCourse> implements JwTBasecourseDao {
    public JwTBasecourseDaoImpl() {
        super(BaseCourse.class);
        // TODO Auto-generated constructor stub
    }
}