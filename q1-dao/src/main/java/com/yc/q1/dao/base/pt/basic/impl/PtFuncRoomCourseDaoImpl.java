package com.yc.q1.dao.base.pt.basic.impl;

import org.springframework.stereotype.Repository;

import com.yc.q1.core.dao.BaseDaoImpl;
import com.yc.q1.dao.base.pt.basic.PtFuncRoomCourseDao;
import com.yc.q1.model.base.pt.basic.PtFuncRoomCourse;


/**
 * 
 * ClassName: JwFuncroomcourseDaoImpl
 * Function: TODO ADD FUNCTION. 
 * Reason: TODO ADD REASON(可选). 
 * Description: 排课课程表(JW_T_FUNCROOMCOURSE)实体Dao接口实现类.
 * date: 2017-03-06
 *
 * @author  luoyibo 创建文件
 * @version 0.1
 * @since JDK 1.8
 */
@Repository("PtFuncRoomCourseDao")
public class PtFuncRoomCourseDaoImpl extends BaseDaoImpl<PtFuncRoomCourse> implements PtFuncRoomCourseDao {
}