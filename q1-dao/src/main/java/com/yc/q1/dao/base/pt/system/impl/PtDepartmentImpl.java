package com.yc.q1.dao.base.pt.system.impl;

import org.springframework.stereotype.Repository;

import com.yc.q1.core.dao.BaseDaoImpl;
import com.yc.q1.dao.base.pt.system.PtDepartmentDao;
import com.yc.q1.model.base.pt.system.PtDepartment;


/**
 * 
 * ClassName: BaseOrgDaoImpl
 * Function: TODO ADD FUNCTION. 
 * Reason: TODO ADD REASON(可选). 
 * Description: BASE_T_ORG实体Dao接口实现类.
 * date: 2016-07-26
 *
 * @author  luoyibo 创建文件
 * @version 0.1
 * @since JDK 1.8
 */
@Repository("PtDepartmentDao")
public class PtDepartmentImpl extends BaseDaoImpl<PtDepartment> implements PtDepartmentDao {
}