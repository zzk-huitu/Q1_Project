 ...
 */
@Controller
@RequestMapping("/PtDeptJob")
public class PtDeptJobController extends FrameWorkController<PtDeptJob> implements Constant {
	...
	public void doDelete(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		if (StringUtils.isEmpty(delIds)) {
			...
			// 判断这些部门岗位是否正在被其他用户使用
			String hql = "select count(a.id) from PtUserDeptJob as a where a.deptJobId in ('" + delIds.replace(",", "','")
					+ "') and a.isDelete=0";
