...
public class PtBaseCourseController extends FrameWorkController<PtBaseCourse> implements Constant {
	...
	@RequestMapping(value = "/listNoPage", method = { org.springframework.web.bind.annotation.RequestMethod.GET,
			org.springframework.web.bind.annotation.RequestMethod.POST })
	public void listNoPage(@ModelAttribute PtBaseCourse entity, HttpServletRequest request,
			HttpServletResponse response) throws IOException {
		String strData = ""; // 返回给js的数据
		QueryResult<PtBaseCourse> qr = thisService.queryPageResult(super.start(request), 0, super.sort(request),
				super.filter(request), true);
	...
	public void doAdd(PtBaseCourse entity, HttpServletRequest request, HttpServletResponse response)
		...
		entity.setId(keyRedisService.getId(PtBaseCourse.ModuleType)); // 手动设置id
	...
	public void doDelete(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		if (StringUtils.isEmpty(delIds)) {
			...
			// 判断这些课程是否被部门学科中绑定
			String hql = "select count(a.id) from PtDepartment as a where a.courseId in ('" + delIds.replace(",", "','")
					+ "') and a.isDelete=0";
