 ...
 */
@Controller
@RequestMapping("/PtDepartment")
public class PtDepartmentController extends FrameWorkController<PtDepartment> implements Constant {

	@Resource
	private PtDepartmentService thisService; // service层接口
	...
	/**
	 * 根据用户的权限，来显示具备权限的树 注：将每个人的权限树的值，存入到redis中；在修改部门权限或指定部门岗位时删除redis数据。
	...
	public void getUserRightDeptTree(HttpServletRequest request, HttpServletResponse response) throws IOException {
		String strData = "";
		// 得到根节点ID
	...
	@RequestMapping(value = { "/getUserRightDeptIds" }, method = {
			org.springframework.web.bind.annotation.RequestMethod.GET,
			org.springframework.web.bind.annotation.RequestMethod.POST })
	public void getUserRightDeptIds(HttpServletRequest request, HttpServletResponse response) throws IOException {

		PtUser currentUser = getCurrentSysUser();

		List<DepartmentTree> baseOrgList = thisService.getUserRightDeptTreeList(currentUser);
		String deptIds = baseOrgList.stream().filter((x) -> x.getIsRight().equals("1")).map((x) -> x.getId())
	...
	public void doAdd(PtDepartment entity, HttpServletRequest request, HttpServletResponse response)
		...
		// 获取同一级别的顺序号
		String hql = " from PtDepartment where orderIndex = (select max(o.orderIndex) from PtDepartment o where  o.isDelete='0' and o.parentNode='"
				+ parentNode + "' )";
	...
	public void doUpdate(PtDepartment entity, HttpServletRequest request, HttpServletResponse response)
			throws IOException, IllegalAccessException, InvocationTargetException {
		 ...
		 * if(!baseOrg.getParentNode().equals(parentNode)){//编辑时改变了它的上级部门，
		 * 需要重新设置orderIndex // 获取同一级别的最大的顺序号 String hql =
		 * " from BaseOrg where orderIndex = (select max(o.orderIndex) from BaseOrg o where  o.isDelete='0' and o.parentNode='"
		 ...
		 * defaultOrderIndex = 0; entity.setOrderIndex(defaultOrderIndex);
		 * }else{ entity.setOrderIndex(baseOrg.getOrderIndex()); }
		 */
		PtUser currentUser = getCurrentSysUser();
