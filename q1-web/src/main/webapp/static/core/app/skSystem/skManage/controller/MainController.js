Ext.define("core.skSystem.skManage.controller.MainController",{
	extend : "Ext.app.ViewController",
	alias : 'controller.skSystem.skManage.mainController',
	mixins :{
	...
	control :{
			"basetreegrid[xtype=skSystem.skManage.roomInfoTree] " : {
			/*
			 * 当点击了这个树的子项后，在查询列表的条件中，要做如下工作： 1. 附带树节点的相关参数 2.
			 * 当存在basegrid的默认参数，则附带上去 3.
			 * 附带快速搜索中的参数（为了防止文本框的数据与实际查询的数据不一致，所以在下面代码中主动获取了文本框的数据）
			...
			// 区域列表刷新按钮事件
			"basetreegrid[xtype=skSystem.skManage.roomInfoTree] button[ref=gridRefresh]" : {
				click : function(btn) {
			...
			// 绑定列表事件
			"basegrid[xtype=skSystem.skManage.mainGrid] actioncolumn" : {
			...
			"basegrid[xtype=skSystem.skManage.mainGrid] button[ref=gridExport]" : {
	...
	doExport : function(btn) {
		...
		var mainlayout = baseGrid.up("panel[xtype=skSystem.skManage.mainLayout]");
		var roominfotreegrid = mainlayout.down("panel[xtype=skSystem.skManage.roomInfoTree]");
		...
		Ext.Msg.confirm('提示',title,function(btn, text) {
			if (btn == "yes") {
				...
				var component = Ext.create('Ext.Component',{
									...
									height : 0,
									hidden : true,
									html : '<iframe src="'+ comm.get('baseUrl')+ '/PtTerm/doExportExcel?termName='+ termName
										+ '&roomId='+ roomId+'&termTypeId=8&roomLeaf='+ roomLeaf+ '"></iframe>',
		...
		return false;
					...
					openHighParamDetail : function(grid, record, cmd) {
						...
						if (termTypeId == "9" || termTypeId == "4") {
							// 9电控，4门禁
							xItemType = "skSystem.skManage.highParamForm";
						} else {
						...
						var detLayout = "skSystem.skManage.detailLayout"; // 打开的tab页的布局视图
					...
					/*
					 * 此方法用于打开所有功能的基础数据界面，并且根据类型去初始化数据； 通过各个参数名称可以快速找到具体参数的设置方法
					 */
					openBaseParamDetail : function(grid, record, cmd) {
						...
						if (termTypeId == '9') {
							// 电控基础参数
							xItemType = "skSystem.skManage.dkbaseparamform";
						} else if (termTypeId == '4') {
							// 门禁基础参数
							xItemType = "skSystem.skManage.doorcontrolform";
						} else if (termTypeId == '17') {
							// 17灯控开关
							xItemType = "skSystem.skManage.ampcontrolform";
						} else if (termTypeId == '11') {
							// 11红外
							xItemType = "skSystem.skManage.infraredparamform";
						} else if (termTypeId == '8') {
							// 水控基础参数
							xItemType = "skSystem.skManage.skBaseParamForm";
						} else {
											...
											self
													.asyncAjax({
														...
														success : function(
															...
															var valStr = null;
															var controlVal = null;
															var checkboxgroup = objForm
																	.down("checkboxgroup[ref=KrateForm_lblOperationBehaviors]");
														...
														success : function(
															...
															for (var i = 0; i < data.length; i++) {
																...
																if (data[i].tag == 0x2007) {
																	...
																	for (var l = 0; l < 4; l++) {
																		var check = objForm
																				.down("checkboxgroup[ref=sKBaseParamForm_lblOperationBehaviors"
																						+ (l + 1)
																						+ "]");
																		if (check != null) {
																	...
																	var val = data[i].valStr
																			.split("|");
																	for (var k = 0; k < 4; k++) {
																		var numitems = objForm
																				.down("container[ref=termparam.KrateForm"
																						+ (k + 1)
																						+ "]");
																		if (numitems != null) {
																			for (var j = 0; j < numitems.items.items.length; j++) {
																				numitems.items.items[j]
																						.setValue(val[0]);
																				val
																						.shift(0);
																			}
													...
													.asyncAjax({
														...
														success : function(
															...
															if (data.tlvs) {
																	...
																	if (data.tlvs[k].tag == 0x1006) {
																		...
																		for (var i = 0; i < item.length; i++) {
																			...
																			for (var l = 0; l < time.length; l++) {
																				...
																				controlVal = fieldsetCpt
																						.query("field[name='items"
																								+ arr1[0]
																								+ ".status"
																								+ l
																								+ "']");
																...
																if (data.tlvs[2].valInt != null
																		&& data.tlvs[2].valInt != '') {
																	objForm
																			.down(
																					"[ref=tlvsval2Int]")
																			.setValue(
																					String(data.tlvs[2].valInt));
																} else {
																	objForm
																			.down(
																					"[ref=tlvsval2Int]")
	...
	loadMainGridStore : function(tree, record) {
		var mainLayout = tree.up("panel[xtype=skSystem.skManage.mainLayout]");
		...
		// 加载房间的人员信息
		var mianGrid = mainLayout.down("panel[xtype=skSystem.skManage.mainGrid]");
	...
	refreshTreeStore : function(btn) {
		...
		var mainlayout = btn.up("basepanel[xtype=skSystem.skManage.mainLayout]");
		var mianGrid = mainlayout.down("basegrid[xtype=skSystem.skManage.mainGrid]");
		var store = mianGrid.getStore();
		var proxy = store.getProxy();
