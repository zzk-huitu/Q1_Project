Ext.define("core.xfSystem.xfManage.controller.MainController",{
	extend : "Ext.app.ViewController",
	alias : 'controller.xfSystem.xfManage.mainController',
	mixins :{
	...
	control :{
			"basetreegrid[xtype=xfSystem.xfManage.roomInfoTree] " : {
			/*
			...
			// 区域列表刷新按钮事件
			"basetreegrid[xtype=xfSystem.xfManage.roomInfoTree] button[ref=gridRefresh]" : {
				click : function(btn) {
			...
			// 绑定列表事件
			"basegrid[xtype=xfSystem.xfManage.mainGrid] actioncolumn" : {
			...
			"basegrid[xtype=xfSystem.xfManage.mainGrid] button[ref=gridExport]" : {
	...
	doExport : function(btn) {
		...
		var mainlayout = baseGrid.up("panel[xtype=xfSystem.xfManage.mainLayout]");
		var roominfotreegrid = mainlayout.down("panel[xtype=xfSystem.xfManage.roomInfoTree]");
			...
			if (btn == "yes") {
				...
				var component = Ext.create('Ext.Component',{
									...
									html : '<iframe src="'+ comm.get('baseUrl')+ '/PtTerm/doExportExcel?termName='+ termName
										+ '&roomId='+ roomId+'&termTypeId=0&roomLeaf='+ roomLeaf+ '"></iframe>',
	...
	openHighParamDetail : function(grid, record, cmd) {
		...
		if (termTypeId == "9" || termTypeId == "4") {
			// 9电控，4门禁
			xItemType = "xfSystem.xfManage.highParamForm";
		} else {
		...
		var detLayout = "xfSystem.xfManage.detailLayout"; // 打开的tab页的布局视图
	...
	openBaseParamDetail : function(grid, record, cmd) {
		...
		if (termTypeId == '9') {
			// 电控基础参数
			xItemType = "xfSystem.xfManage.dkbaseparamform";
		} else if (termTypeId == '4') {
			// 门禁基础参数
			xItemType = "xfSystem.xfManage.mjControlForm";
		} else if (termTypeId == '17') {
			// 17灯控开关
			xItemType = "xfSystem.xfManage.ampcontrolform";
		} else if (termTypeId == '11') {
			// 11红外
			xItemType = "xfSystem.xfManage.infraredparamform";
		} else if (termTypeId == '8') {
			// 水控基础参数
			xItemType = "xfSystem.xfManage.skBaseParamForm";
		} else {
						...
						success : function(response) {
									...
									if (data[i].tag == 0x1015) {
											...
											if (controlVal != null) {
												...
												if (controlVal != null)
														controlVal.setValue(data[i].valStr);
																	}
																}
															}
															;
	...
	loadMainGridStore : function(tree, record) {
		var mainLayout = tree.up("panel[xtype=xfSystem.xfManage.mainLayout]");
		...
		// 加载房间的人员信息
		var mianGrid = mainLayout.down("panel[xtype=xfSystem.xfManage.mainGrid]");
		var girdSearchTexts = mianGrid.query("field[funCode=girdFastSearchText]");
		var filter = new Array();
		filter.push({"type" : "string","value" : "0","field" : "termTypeId","comparison" : "="})
	...
	refreshTreeStore : function(btn) {
		...
		var mainlayout = btn.up("basepanel[xtype=xfSystem.xfManage.mainLayout]");
		var mianGrid = mainlayout.down("basegrid[xtype=xfSystem.xfManage.mainGrid]");
		var store = mianGrid.getStore();
		var proxy = store.getProxy();
