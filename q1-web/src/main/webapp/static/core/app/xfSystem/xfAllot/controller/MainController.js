Ext.define("core.xfSystem.xfAllot.controller.MainController", {
    extend: "Ext.app.ViewController",
    alias: 'controller.xfSystem.xfAllot.mainController',
    mixins: {
    ...
    control: {
        ...
        //区域列表刷新按钮
        "basetreegrid[xtype=xfSystem.xfAllot.roomInfoTree] button[ref=gridRefresh]": {
            beforeclick: function(btn) {
                btn.up('basetreegrid').getStore().load();
                var mainlayout = btn.up("basepanel[xtype=xfSystem.xfAllot.mainlayout]");
                var mianGrid = mainlayout.down("basegrid[xtype=xfSystem.xfAllot.maingrid]");
        ...
        //分配设备按钮
    	"basegrid[xtype=xfSystem.xfAllot.mainGrid] button[ref=gridAdd_Tab]": {
            beforeclick: function(btn) {
           	    this.doAllot(btn);
                return false;
            }
    	 ...
    	 "basegrid[xtype=xfSystem.xfAllot.mainGrid] button[ref=gridDelete]": {
            beforeclick: function(btn) {
         ...
         * 操作列的操作事件
         */
         "basegrid[xtype=xfSystem.xfAllot.mainGrid] actioncolumn": {
        ...
        "basegrid[xtype=xfSystem.xfAllot.mainGrid] button[ref=gridExport]": {
            beforeclick: function(btn) {
                this.doExport(btn);
                return false;
            }
    ...
    doAllot: function (btn) {
        ...
        var detCode =  "deviceallot_layout";               //打开的tab也的detCode标识，可自定指定，用于查找唯一组件
      	...
      	if (!otherController)
      		  otherController = '';  
         var itemXtype = "xfSystem.xfAllot.deviceAllotLayout";
         var xItemType=[{
            xtype:itemXtype,
            funCode:detCode
          }];
    ...
    doExport:function(btn){
        var self = this;
        var baseGrid = btn.up("basegrid");
        var mainlayout=baseGrid.up("panel[xtype=xfSystem.xfAllot.mainLayout]");
       
        var roominfotreegrid=mainlayout.down("panel[xtype=xfSystem.xfAllot.roomInfoTree]");
        ...
        var userGrid = mainlayout.down("basegrid[xtype=xfSystem.xfAllot.mainGrid]");
        //获取快速搜索栏数据
        ...
        //获取高级搜索栏数据
        var queryPanel = userGrid.down("basequeryform[xtype=xfSystem.xfAllot.mainQueryPanel]"); 
        ...
        Ext.Msg.confirm('提示', title, function (btn, text) {
            if (btn == "yes") {
                ...
                var component = Ext.create('Ext.Component', {
                    ...
                    height: 0,
                    hidden: true,
                    html: '<iframe src="' + comm.get('baseUrl') + '/PtTerm/doExportPtTermAllotExcel?termSn='+termSn+'&roomId='+roomId+'&termSN='+termSN+'&termTypeId=0&termNo='+termNo+'&termName='+termName+'&roomLeaf='+roomLeaf+'"></iframe>',
