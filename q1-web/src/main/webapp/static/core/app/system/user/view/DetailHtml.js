Ext.define("core.system.user.view.DetailHtml", {
    ...
    items: [{
        ...
        tpl: new Ext.XTemplate(
            ...
            '<li style="width:50%" title="{name}">真实姓名：{xm}</li>',
            '<li style="width:50%">性别：',
            '<tpl if="sex==1">男<tpl elseif="sex==2">女</tpl></li>',
            ...
            '<li style="width:50%" title="{identityNumber}">身份证件号：{sfzjh}</li>',
            '<li style="width:50%" title="{politicalStatus}">政治面貌：{zzmmm}</li>',
            '<li style="width:50%" title="{headCountType}">编制：{zxxbzlb}</li>',
            '<li style="width:50%" title="{birthDate}">出生日期：{csrq}</li>',
            '<li style="width:50%" title="{mobilePhone}">移动电话：{mobile}</li>',
            '<li style="width:50%" title="{email}">电子邮箱：{dzxx}</li>',
