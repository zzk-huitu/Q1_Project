Ext.define("core.baseset.terminal.view.DetailHtml", {
    ...
    tpl: new Ext.XTemplate(
        ...
        '<ul>' ,
        '<li style="width:50%" title="{terminalNo}">终端号：{terminalNo}</li>',
        '<li style="width:50%" title="{terminalType}">终端类型：{terminalType}</li>',
        '<li style="width:50%">使用状态：<tpl if="isUse == 1">已使用<tpl else>未使用</tpl></li>',
        '<li style="width:50%" title="{roomName}">房间名称：{roomName}</li>',
        '<li style="width:50%" title="{terminalSpec}">规格：{terminalSpec}</li>',
