Ext.define("core.dkSystem.dkManage.controller.MainController",{
	extend : "Ext.app.ViewController",
	alias : 'controller.dkSystem.dkManage.mainController',
	mixins :{
	...
	control :{
			"basetreegrid[xtype=dkSystem.dkManage.roomInfoTree] " : {
			/*
			...
			// 区域列表刷新按钮事件
			"basetreegrid[xtype=dkSystem.dkManage.roomInfoTree] button[ref=gridRefresh]" : {
				click : function(btn) {
			...
			// 绑定列表事件
			"basegrid[xtype=dkSystem.dkManage.mainGrid] actioncolumn" : {
			...
			"basegrid[xtype=dkSystem.dkManage.mainGrid] button[ref=gridExport]" : {
	...
	doExport : function(btn) {
		...
		var mainlayout = baseGrid.up("panel[xtype=dkSystem.dkManage.mainLayout]");
		var roominfotreegrid = mainlayout.down("panel[xtype=dkSystem.dkManage.roomInfoTree]");
			...
			if (btn == "yes") {
				...
				var component = Ext.create('Ext.Component',{
									...
									html : '<iframe src="'+ comm.get('baseUrl')+ '/PtTerm/doExportExcel?termName='+ termName
										+ '&roomId='+ roomId+'&termTypeId=9&roomLeaf='+ roomLeaf+ '"></iframe>',
		...
		return false;
					...
					openHighParamDetail : function(grid, record, cmd) {
						...
						if (termTypeId == "9" || termTypeId == "4") {
							// 9电控，4门禁
							xItemType = "dkSystem.dkManage.highParamForm";
						} else {
						...
						var detLayout = "dkSystem.dkManage.detailLayout"; // 打开的tab页的布局视图
					...
					openBaseParamDetail : function(grid, record, cmd) {
						...
						if (termTypeId == '9') {
							...
							// 17灯控开关
							xItemType = "dkSystem.dkManage.ampcontrolform";
						} else if (termTypeId == '11') {
							// 11红外
							xItemType = "dkSystem.dkManage.infraredparamform";
						} else if (termTypeId == '8') {
							// 水控基础参数
							xItemType = "dkSystem.dkManage.skBaseParamForm";
						} else {
	...
	loadMainGridStore : function(tree, record) {
		var mainLayout = tree.up("panel[xtype=dkSystem.dkManage.mainLayout]");
		...
		// 加载房间的人员信息
		var mianGrid = mainLayout.down("panel[xtype=dkSystem.dkManage.mainGrid]");
	...
	refreshTreeStore : function(btn) {
		...
		var mainlayout = btn.up("basepanel[xtype=dkSystem.dkManage.mainLayout]");
		var mianGrid = mainlayout.down("basegrid[xtype=dkSystem.dkManage.mainGrid]");
		var store = mianGrid.getStore();
		var proxy = store.getProxy();
