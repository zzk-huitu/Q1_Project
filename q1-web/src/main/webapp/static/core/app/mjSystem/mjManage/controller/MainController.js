Ext.define("core.mjSystem.mjManage.controller.MainController",{
	extend : "Ext.app.ViewController",
	alias : 'controller.mjSystem.mjManage.mainController',
	mixins :{
	...
	control :{
			"basetreegrid[xtype=mjSystem.mjManage.roomInfoTree] " : {
			/*
			...
			// 区域列表刷新按钮事件
			"basetreegrid[xtype=mjSystem.mjManage.roomInfoTree] button[ref=gridRefresh]" : {
				click : function(btn) {
			...
			// 绑定列表事件
			"basegrid[xtype=mjSystem.mjManage.mainGrid] actioncolumn" : {
			...
			"basegrid[xtype=mjSystem.mjManage.mainGrid] button[ref=gridExport]" : {
	...
	doExport : function(btn) {
		...
		var mainlayout = baseGrid.up("panel[xtype=mjSystem.mjManage.mainLayout]");
		var roominfotreegrid = mainlayout.down("panel[xtype=mjSystem.mjManage.roomInfoTree]");
			...
			if (btn == "yes") {
				...
				var component = Ext.create('Ext.Component',{
									...
									html : '<iframe src="'+ comm.get('baseUrl')+ '/PtTerm/doExportExcel?termName='+ termName
										+ '&roomId='+ roomId+'&termTypeId=4&roomLeaf='+ roomLeaf+ '"></iframe>',
	...
	openHighParamDetail : function(grid, record, cmd) {
		...
		if (termTypeId == "9" || termTypeId == "4") {
			// 9电控，4门禁
			xItemType = "mjSystem.mjManage.highParamForm";
		} else {
		...
		var detLayout = "mjSystem.mjManage.detailLayout"; // 打开的tab页的布局视图
	...
	openBaseParamDetail : function(grid, record, cmd) {
		...
		if (termTypeId == '9') {
			// 电控基础参数
			xItemType = "mjSystem.mjManage.dkbaseparamform";
		} else if (termTypeId == '4') {
			// 门禁基础参数
			xItemType = "mjSystem.mjManage.mjControlForm";
		} else if (termTypeId == '17') {
			// 17灯控开关
			xItemType = "mjSystem.mjManage.ampcontrolform";
		} else if (termTypeId == '11') {
			// 11红外
			xItemType = "mjSystem.mjManage.infraredparamform";
		} else if (termTypeId == '8') {
			// 水控基础参数
			xItemType = "mjSystem.mjManage.skBaseParamForm";
		} else {
	...
	loadMainGridStore : function(tree, record) {
		var mainLayout = tree.up("panel[xtype=mjSystem.mjManage.mainLayout]");
		...
		// 加载房间的人员信息
		var mianGrid = mainLayout.down("panel[xtype=mjSystem.mjManage.mainGrid]");
	...
	refreshTreeStore : function(btn) {
		...
		var mainlayout = btn.up("basepanel[xtype=mjSystem.mjManage.mainLayout]");
		var mianGrid = mainlayout.down("basegrid[xtype=mjSystem.mjManage.mainGrid]");
		var store = mianGrid.getStore();
		var proxy = store.getProxy();
